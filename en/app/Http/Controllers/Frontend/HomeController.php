...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        // dd($casinos, nj_get_current_country());
        //->whereRaw("FIND_IN_SET('IN', countries")
        //->whereIn('id', explode(',', $casinos_ids))
        //->orderByRaw("FIELD(id, $casinos_ids)")
        ...
        /* 
        $games_ids = implode(',', @json_decode(StaticPage::getField('home', 'games'), true) ?? []);
        $games = Game::select('id','name','logo_alt_text')
            ->whereRaw("`id` IN ($games_ids)")
            ->orderByRaw("FIELD(id, $games_ids)");

        if($current_country){
            $games = $games->countries([$current_country]);
        }

        $games = $games_ids ? $games->get(): null;

        $blog_ids = implode(',', @json_decode(StaticPage::getField('home', 'blog'), true) ?? []);
        $blog = News::select('id','name', 'small_description', 'logo_img_alt_text')
            ->whereRaw("`id` IN ($blog_ids)")
            ->orderByRaw("FIELD(id, $blog_ids)");

        if($current_country){
            $blog = $blog->countries([$current_country]);
        }

        $blog = $blog_ids ? $blog->get(): null;
        */
        ...
        return view('frontend.index', compact('slider_casinos', 'casinos', 'top3_casinos', 'games', 'blog', 'faq_questions', 'home_content','newsletter'));
    }
