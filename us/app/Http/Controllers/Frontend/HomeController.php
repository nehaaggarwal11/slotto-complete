...
class HomeController extends Controller
{
    ...
    public function home()
    {
        ...
        $casinos = $casinos_ids ? Casino::select('id','name','spins', 'spins_text','bonus',
        ...
        $games = $games_ids ? Game::select('id','name','logo_alt_text')
            ->whereRaw("`id` IN ($games_ids)")
            ->orderByRaw("FIELD(id, $games_ids)")
            ->get(): null;

        $blog_ids = implode(',', @json_decode(StaticPage::getField('home', 'blog'), true) ?? []);
        $blog = $blog_ids ? News::select('id','name', 'small_description', 'logo_img_alt_text')
        ...
        return view('frontend.home', compact('slider_casinos', 'casinos', 'top3_casinos', 'games', 'blog', 'faq_questions', 'home_content', 'newsletter'));
    }
