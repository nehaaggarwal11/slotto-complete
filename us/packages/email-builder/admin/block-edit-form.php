...
$info=$db->get_template_blockById($id);
  ...
  <div class="">
    <div class="row">
      <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
          <div class="x_title">
            <h2>Edit template block </h2>
            <div class="clearfix"></div>
          </div>
          <div class="x_content">
            <form class="form-horizontal form-label-left block-edit-form"  data-id="<?php echo $id; ?>" novalidate>
              <div class="item form-group">
                    ...
                    <select id="category"  name="category" class="form-control col-md-7 col-xs-12" >
                      ...
                      <?php
                          for ($i=0; $i < sizeof($blocks_category); $i++) {
                              ...
                              echo '<option value="'.$blocks_category[$i]['id'].'" '.($blocks_category[$i]['id']==$info[0]['cat_id']?'selected':'').' >'.$blocks_category[$i]['name'].'</td>';
                          }
                       ?>
                    </select>
                  </div>
                ...
                <div class="col-md-6 col-sm-6 col-xs-12">
                  <input id="name" class="form-control col-md-7 col-xs-12"  name="name" value="<?php echo $info[0]['name'] ?>"
                  ...
                  <select id="icon" name="icon" required="required" class="form-control col-md-7 col-xs-12">
                    <?php
                      ...
                      foreach ($iconList as $key => $value) {
                        echo '<option value="'.$key.'"  '.($key==$info[0]['icon']?'selected="selected"':'').'>'.$value.'</option>';
                      }
                     ?>
                  </select>
                </div>
              ...
              <div class="item form-group">
                ...
                <div class="col-md-6 col-sm-6 col-xs-12">
                  <select id="block_property" multiple="multiple" class="form-control col-md-7 col-xs-12">
                       <?php
                           $selectedProperties=explode(',',$info[0]['property']);
                           for ($i=0; $i < sizeof($selectedProperties); $i++) {
                               echo '<option selected="selected" value="'.$selectedProperties[$i].'" >'.$selectedProperties[$i].'</td>';
                           }
                        ?>
                  </select>
                </div>
              ...
              <div class="item form-group">
                ...
                <div class="col-md-6 col-sm-6 col-xs-12">
                        <textarea name="editor" id="editor"><?php echo $info[0]['html'] ?></textarea>
                </div>

                </div>
                <br>
                <div class="item form-group">
                  <label class="control-label col-md-3 col-sm-3 col-xs-12" for="isUpdate">
                  </label>
                  <div class="col-md-6 col-sm-6 col-xs-12">
                    <label  >
                    <input type="checkbox" name="isUpdate" id="isUpdate"   class="flat" />
                    Update all templates which are using this block
                      </label>
                      <div class="text-mute">
                          - if you check this feature, all templates which are using this block, will be updated
                      </div>
                  </div>
                </div>
              </div>
