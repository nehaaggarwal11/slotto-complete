...
class CustomUserRepository extends Auth0UserRepository
{
    ...
    protected function upsertUser( $profile ) {
        ...
        $db_admin_role_id = config('panel.admin_role_id');
        $auth0_admin_role_id = config('panel.auth0_admin_role_id');

        $user_roles = collect((new Auth0Helper())->getUserRoles([
            'user' => $profile['sub']
        ]))->pluck('id')->toArray();

        $is_admin = in_array($auth0_admin_role_id, $user_roles);
        ...
        if($is_admin){
            $user->roles()->sync([$db_admin_role_id]);
        }
